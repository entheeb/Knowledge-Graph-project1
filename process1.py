...
def get_idx(path):
    ...
    for split in ["train.txt", "valid.txt", "test.txt"]:#"test_rel0","test_rel1","test_rel2","test_rel3","test_rel4","test_rel5","test_rel6","test_rel7","test_rel8","test_rel9","test_rel10"]:
        with open(os.path.join(path, split), "r") as lines:
            for line in lines:
                ...
                entities.add(rhs)
                relations.add(rel)
    ent2idx = {x: x for (i, x) in enumerate(sorted(entities))}
    rel2idx = {x: x for (i, x) in enumerate(sorted(relations))}
...
def process_dataset(path, evaluation_step):
    ...
    examples = {}
    #TODO: if evalutation_step=='valif':
    # splits = [train]
    #else:
    # splits= [train,valid]
    splits = ["train.txt", "valid.txt"]#"test_rel0","test_rel1","test_rel2","test_rel3","test_rel4","test_rel5","test_rel6","test_rel7","test_rel8","test_rel9","test_rel10"]
    #splits = ["test.txt"]
    for split in splits:
...
if __name__ == "__main__":
    ...
    dataset_path = os.path.join(data_path, "FB15k-237-CQA")
    dataset_examples, dataset_filters = process_dataset(dataset_path)
    print(dataset_path)
    #for dataset_split in ["train.txt", "valid.txt", "test.txt"]:# ["train", "valid", "test_rel0","test_rel1","test_rel2","test_rel3","test_rel4","test_rel5","test_rel6","test_rel7","test_rel8","test_rel9","test_rel10"]:
    #    save_path = os.path.join(dataset_path, dataset_split + ".pickle")
    #    with open(save_path, "wb") as save_file:
    #        pickle.dump(dataset_examples[dataset_split], save_file)
    with open(os.path.join(dataset_path, "to_skip_valid.pickle"), "wb") as save_file:
